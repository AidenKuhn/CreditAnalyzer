...
function App() {
  ...
  const submitCreditData = async () => {
    ...
    if (!income || debt === undefined || !age || creditHistory === undefined || !paymentHistory) {
    ...
    if (income <= 0 || debt < 0 || creditHistory < 0) {
      showMessage('Invalid financial data entered!', 'error')
      return
    }
    ...
    try {
      ...
      console.log('📋 Data:', { income, debt, age, creditHistory, paymentHistory })

      // Execute transaction with full tracking
      ...
      showMessage(error.message || 'Failed to submit credit data. Please try again.', 'error')
    } finally {
      setLoading(false)
    }
  }
